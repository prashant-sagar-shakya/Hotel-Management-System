	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == b1) {
			try (conn c = new conn()) {
				...
				String str = "SELECT * FROM customer WHERE id_number = ?"; // Use 'id_number'
				...
				if (rs.next()) {
					...
					try (PreparedStatement ps2 = c.c.prepareStatement("SELECT price FROM room WHERE room_number = ?")) {
				...
				System.out.println("Error checking customer details: " + e.getMessage());
				JOptionPane.showMessageDialog(null, "Error checking customer details. Please try again.");
			}
