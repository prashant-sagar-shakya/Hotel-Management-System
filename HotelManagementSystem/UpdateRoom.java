...
public class UpdateRoom extends JFrame implements ActionListener {
	...
	UpdateRoom() {
		...
		try (conn c = new conn()) {
			...
			while (rs.next()) {
				c1.add(rs.getString("id_number")); // Use 'id_number'
			}
		} catch (Exception e) {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == b1) {
			...
			try (conn c = new conn()) {
				ResultSet rs = c.s.executeQuery("select * from customer where id_number = '" + s1 + "'"); // Use 'id_number'
				while (rs.next()) {
					t1.setText(rs.getString("room"));
					s2 = rs.getString("room");
				}

				if (s2 != null) { // Check if a room was found
					...
					JOptionPane.showMessageDialog(null, "Room not found for this Guest ID.");
				}
			} catch (Exception e) {
				System.out.println(e);
			}
			...
			try (conn c = new conn()) {
				...
				String str = "UPDATE room SET availability = ?, status = ? WHERE room_number = ?";
				...
				JOptionPane.showMessageDialog(null, "Error updating room. Please try again.");
			}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> new UpdateRoom());
	}
}