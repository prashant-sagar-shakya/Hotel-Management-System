...
public class PickUp extends JFrame implements ActionListener {
	...
	PickUp() {
		...
		try (conn c = new conn()) {
			...
			System.out.println("Error loading vehicle models: " + e.getMessage());
		} catch (Exception e) {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == b1) {
			try (conn c = new conn()) {
				String str = "select * from driver where car_model = ?";
				...
				System.out.println("Error loading driver data: " + e.getMessage());
				JOptionPane.showMessageDialog(null, "Error loading driver data. Please try again.");
			}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> new PickUp());
	}
}